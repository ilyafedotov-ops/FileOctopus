...
## Extended review — Settings dialog and menus (2026-06-11)

Same method: live browser preview at 1440×900, all 14 settings categories and all 7 menu-bar menus plus the file context menu.

### S. Settings dialog

| #   | Finding                                                                                                                                                                                                                                                                                                                                                                    | Severity    | Status                                                                     |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------- |
| S1  | All dialog size presets were **dead CSS**: `.fo-settings-dialog` (48 rem + stable height), `.fo-about-dialog`, `.fo-diagnostics/go-to/error-details` lose the cascade to `dialog.fo-dialog` (specificity 0,1,1 vs 0,1,0). Settings rendered 560 px wide with the height jumping on every category switch — the exact resize the stable-height rule was written to prevent. | 🔴 Critical | Fixed — presets re-anchored as `.fo-dialog.fo-*`                           |
| S2  | Network category "Provider availability" rendered as run-together text ("SFTPFiles + terminalPort 22") — `fo-settings-provider-list/-row` classes had **no CSS rule**.                                                                                                                                                                                                     | 🔴 Critical | Fixed — 3-column grid                                                      |
| S3  | Colors category: file-type rule rows clipped at the right edge with stray scrollbars (Edit/Delete cut off).                                                                                                                                                                                                                                                                | 🟡 Moderate | Fixed by S1 (dialog now actually 48 rem wide)                              |
| S4  | Search filters the category nav but the content panel keeps showing the previously selected (now hidden) category.                                                                                                                                                                                                                                                         | 🟡 Moderate | Fixed — first match auto-selected when the active category is filtered out |
| S5  | `DialogShell`'s `size="sm/md/lg"` prop emitted `fo-dialog--{size}` classes that had **no CSS anywhere** — the prop was a no-op.                                                                                                                                                                                                                                            | 🟡 Moderate | Fixed — size classes defined (26/34/48 rem)                                |
| S6  | Keyboard category: mixed modifier notation (⌥← vs ^R vs lowercase "backspace"); File List ships identical "Default" and "Detailed" presets; Operations "Fail" conflict-policy label is developer jargon.                                                                                                                                                                   | 🟢 Minor    | Backlog (UPP-L8)                                                           |

### M. Menu bar and context menus

| #   | Finding                                                                                                                                                                                                                                                                  | Severity    | Status                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| M1  | Shortcut notation inconsistent: Edit/File used platform glyphs (⌘X, ⇧Delete) via `menuShortcut()`, but Go/View/Window/Help/File-footer hardcoded Windows-style strings ("Alt+←", "Ctrl+L", "Ctrl+U", "Ctrl+/", "Ctrl+,", "Ctrl+Q").                                      | 🟡 Moderate | Fixed — all via `menuShortcut()`/`platformShortcut()`; Go → Home dropped its never-bound "Alt+Home"                                   |
| M2  | `DropdownMenuItem.checked` **rendered nothing** — the only CSS rule for `--checked` styled an empty icon span. Every "Show Sidebar/Toolbar/Status Bar/Dual Pane" checkmark silently never displayed, and view mode / sort / theme / density had no state plumbed at all. | 🔴 Critical | Fixed — ✓ glyph + `role="menuitemcheckbox"` + `aria-checked`; view mode, sort field/direction, theme, density now plumbed and checked |
| M3  | View menu too long (scrolled at 900 px) with "Theme: Light"-style prefix items.                                                                                                                                                                                          | 🟡 Moderate | Fixed — Theme and Density are submenus                                                                                                |
| M4  | Diagnostics + Export Diagnostics duplicated in Tools and Help; context menu has two "Copy" entries (clipboard vs path-submenu); menu surfaces show an always-visible scrollbar.                                                                                          | 🟢 Minor    | Backlog (UPP-L8)                                                                                                                      |
