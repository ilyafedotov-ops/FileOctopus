...
- Source: `crates/app-core/src/lib.rs`
- Depends on: `vfs`, `fs-core`, `jobs`, `telemetry`, `rusqlite`, `chrono`, `uuid`, `thiserror`.
- Used by: `apps/desktop-tauri/src-tauri/src/lib.rs` (as `tauri::Manager` state) and integration tests in `crates/app-core/tests/` (where present).
