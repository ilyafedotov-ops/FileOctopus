...
| UPP-L6 — style the KNOWN_UNSTYLED components  | ⬜ Backlog | MultiRenameDialog, SessionManagerDialog, NetworkLocationsDialog profile list, ContentSearchPanel, tag chips, ACL editor accents, etc. Burn down `KNOWN_UNSTYLED` in `classContract.test.ts` to zero.                                             |
...
| UPP-L8 — settings polish leftovers            | ⬜ Backlog | Keyboard category mixed modifier notation; duplicate "Default"/"Detailed" column presets; "Fail" conflict-policy label copy; Tools↔Help diagnostics duplication; menu scrollbar always visible.                                                  |
