...
| UPP-L8 — settings polish leftovers            | 🟨 Mostly done | Done: duplicate "Detailed" column preset removed (identical to Default — only 5 columns exist); conflict-policy labels humanized ("Stop and report", "Keep both (rename new)", …); diagnostics entries deduplicated (kept under Help, removed from Tools). Remaining: Keyboard category mixed modifier notation; menu scrollbar always visible.                                                                                                                     |
