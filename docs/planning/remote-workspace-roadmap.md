...
| B — Remote Operation Pipeline for SFTP | **Mostly done**    | `crates/fs-core/src/vfs_io.rs` is a 900-line VFS-aware I/O facade: `mkdir`, `create_empty_file`, `rename`, `remove` (recursive), and `copy_file` across all four scheme combinations (local↔local/sftp↔sftp/local→sftp/sftp→local) with progress callbacks. `OperationRuntime` (in `crates/app-core/src/lib.rs:170`) constructs `VfsFilesystem::with_sessions(...)` so planned operations flow through it. SFTP blocking helpers in `crates/provider-sftp/src/ops.rs` cover upload/download/rename/mkdir/remove | Trash and symlink-copy still return `Unsupported*` for remote; archive/zip, `fs_compute_hash`, editor open/save (`read_file_range`/`write_file_atomic`) remain local-only. Architectural cleanup: writes live on `VfsFilesystem` rather than the `VfsProvider` trait — deferred to Phase 5 |
