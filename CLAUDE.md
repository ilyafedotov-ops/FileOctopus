...
- **`@fileoctopus/frontend`** (`packages/frontend/`) — the `FileOctopusShell` React component (two-panel UI, operation toolbar, activity/history panel) plus `panelStore.ts` (`useReducer`-based state, virtualized rows, `normalizeLocalInput`). File-op handlers are composed in `packages/frontend/src/hooks/useFileOpHandlers.ts` from per-operation hooks under `hooks/fileOps/{useClipboardHandlers,useMutationHandlers,useTransferHandlers,useMetadataHandlers,useArchiveHandlers}.ts`.
